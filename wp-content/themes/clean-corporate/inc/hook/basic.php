...
if ( ! function_exists( 'clean_corporate_implement_excerpt_length' ) ) :
	...
	function clean_corporate_implement_excerpt_length( $length ) {
		...
		return apply_filters( 'clean_corporate_filter_excerpt_length', esc_attr( $excerpt_length ) );
...
if ( ! function_exists( 'clean_corporate_import_custom_css' ) ) :
	 ...
	 * @since 1.0.5
	 */
	function clean_corporate_import_custom_css() {
		...
		if ( ! is_wp_error( $return ) ) {
			...
			// Remove from theme.
			$options = clean_corporate_get_options();
			$options['custom_css'] = '';
...
add_action( 'after_setup_theme', 'clean_corporate_import_custom_css', 99 );
